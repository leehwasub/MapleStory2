package utils;

import java.awt.Color;

public class ColorUtils {
	public static final Color SEA = new Color(0, 128, 255);
	public static final Color SEA_40 = new Color(0, 128, 255, 102);
	public static final Color BROWN = new Color(150, 75, 0);
	public static final Color NAVY = new Color(8, 11, 84);
	public static final Color PURPLE = new Color(128, 0, 255);
	public static final Color LEMON = new Color(255, 244, 79);
	public static final Color GOLD = new Color(255, 215, 0);
	public static final Color PANEL_BORDER = new Color(67, 67, 67);
	public static final Color PANEL_CENTER = new Color(54, 46, 43);
	public static final Color BLACK_80 = new Color(0, 0, 0, 204);
	public static final Color WHITE_80 = new Color(255, 255, 255, 204);
	public static final Color BLACK_60 = new Color(0, 0, 0, 153);
	public static final Color WHITE_60 = new Color(255, 255, 255, 153);
	public static final Color WHITE_40 = new Color(255, 255, 255, 102);
	public static final Color WHITE_20 = new Color(255, 255, 255, 51);
	public static final Color WHITE_0 = new Color(255, 255, 255, 0);
	public static final Color YELLOW_40 = new Color(255, 255, 0, 102);
	public static final Color YELLOW_60 = new Color(255, 255, 0, 153);
	public static final Color GRAY_40 = new Color(128, 128, 128, 102);
}

...
public class Starling extends Npc{
	...
	public void requestQuest(Player player) {
		Quest quest = null;
		if(questNum == 0) {
			quest = Quest.makeQuest(1, 16, "에레고스 처치 준비").addMessage("폐쇄 구역에서 '세릴'을 만났다. 에릴의 말에 의하면 에레고스를 처치하기 위해서는 샤레니안 궁전에 가야하고 에레고스를 봉인하고 있는 샤레니안 궁전의 결계를")
			...
			quest = Quest.makeQuest(1, 17, "에레고스 처치").addMessage("세릴이 재료를 사용하여 마법 포탈을 만들었고 샤레니안 왕성으로 들어갈 준비는 끝났다. 왕성으로 들어가기 전에 빼먹은 것이 없는지")
			...
			quest = Quest.makeQuest(1, 18, "에레고스 처치 보고").addMessage("에레고스 처치를 완료하고 세릴을 찾아갔다. 세릴은 에레고스를 처치한 사실에 놀라며 윈스턴씨에게 알리고 오겠다고 한다. 덕분에 유물 발굴단 작업이 다시 진행 될 것 이라는데")
					.addMessage("이쪽은 에레고스를 처치한 것 에대해 '주먹 펴고 일어서'에게 전해 주도록 하자.").setRewardExp(100).addRewardItem("페리온을구한자", 1)

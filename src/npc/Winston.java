...
public class Winston extends Npc{
	...
	public void requestQuest(Player player) {
		Quest quest = null;
		if(questNum == 0) {
			quest = Quest.makeQuest(1, 13, "발굴지 몬스터 정리").addMessage("우드마스크에서 발견된 물질에 대해 설명하자 윈스턴씨는 이 이야기를 듣고 놀라는 기색을 보이며 세릴에게 이사실을 전해주고 오겠다고 한다.")
					...
					.addQuestKill("스켈독", 10).addQuestKill("머미독", 10).setRewardExp(1000).setRewardMoney(2000).setPlayerQuestProceed(QuestProceed.WINSTON_QUEST_1)
					.addRewardNpcQuestProceed("세릴", 1).addRewardItem("레몬", 50);
			player.setQuest(quest);
		} else if(questNum == 1) {
			quest = Quest.makeQuest(1, 14, "세릴에게로").addMessage("발굴 캠프장 주변에 몬스터를 정리한 이후 윈스턴과 에레고스 퇴치에 관한 이야기를 했다.")
